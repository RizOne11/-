...
class CatalogScout(MarketplaceScout):
    ...
    async def discover(self,mission:ProductMission,query:str)->list[Offer]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            urls=await self._candidate_urls(client,query); result=[]
            for url in urls:
                try:
...
class ZakupkaScout(CatalogScout):
    marketplace=Marketplace.ZAKUPKA; host="zakupka.com"; allow_subdomains=True; external_fallback=True
    # Zakupka is broad-marketplace coverage. Native discovery is tried first;
    # site-scoped server-side discovery is a fallback when their frontend changes.
    search_templates=("https://zakupka.com/search/?q={q}","https://zakupka.com/uk/search/?q={q}")

# Category-only sources kept in code for later, but paused from active production smoke.
