...
def validate_offer(mission: ProductMission, offer: Offer) -> ValidatedOffer:
    ...
    matched_ids = [i for i in identifiers if _norm(i) and _norm(i) in _norm(offer_text)]
    positive: list[str] = []
    conflicts: list[str] = []

    if matched_ids:
        positive.append("identifier match: " + ", ".join(matched_ids[:4]))
    if overlap >= 0.35:
        positive.append(f"source token overlap={overlap:.2f}")

    # v0.1 is deliberately recall-oriented. Strong identifier agreement wins;
    # ambiguous candidates are preserved as CONFLICT instead of disappearing.
    if matched_ids:
